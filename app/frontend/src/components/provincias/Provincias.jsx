...
import ProvinciasBuscar from "./ProvinciasBuscar";
import ProvinciasListado from "./ProvinciasListado";
import ProvinciasRegistro from "./ProvinciasRegistro";
...
function Provincias() {
 ...
 const [ Provincias, setProvincias ] = useState(null); //?????????
  ...
  async function Buscar(_pagina) {
    ...
    const data = await provinciasService.Buscar(Nombre, Activo, _pagina);
    setItems(data.Items);
  ...
  async function BuscarPorId(item, accionABMC) {
    const data = await provinciasService.BuscarPorId(item);
    setItem(data);
  ...
  async function ActivarDesactivar(item) {
    ...
    modalDialogService.Confirm(
      ...
      undefined,
      async () => {
        await provinciasService.ActivarDesactivar(item);
        await Buscar();
      }
    );
  }
  ...
  async function Grabar(item) {
    // agregar o modificar
    await provinciasService.Grabar(item);
    await Buscar();
    Volver();
  ...
  return (
    <div>
      <div className="tituloPagina">
        Provincias <small>{TituloAccionABMC[AccionABMC]}</small>
      </div>
